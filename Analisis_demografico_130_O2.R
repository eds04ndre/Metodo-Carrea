# ================================================================
# Caracteristicas DEMOGRAFICAS de aciertos vs fallos - Observador 2
# ================================================================
# Replica de Analisis_demografico_130.R sobre resultados/estimaciones_130_O2.csv
# (mediciones individuales del Observador 2). Ejecutar despues de
# Cuerda_geometrica_130_O2.R.
# ================================================================
...
png(file.path(dir_fig, "demo130_O2_aciertos_sexo.png"), width = 1000, height = 700, res = 130)
par(mar = c(4.5, 4.5, 3.5, 1))
m <- tasa_por("Sexo")
bp <- barplot(m, beside = TRUE, col = c(COL_MED, COL_GEO), ylim = c(0, 100),
              ylab = "% que acierta (nivel persona)", xlab = "Sexo",
              main = "Tasa de acierto de Carrea por sexo (O2)")
text(bp, m, labels = sprintf("%.0f%%", m), pos = 3, cex = 0.8)
legend("topright", c("Cuerda medida", "Cuerda geometrica"),
       fill = c(COL_MED, COL_GEO), bty = "n")
dev.off()

png(file.path(dir_fig, "demo130_O2_aciertos_origen.png"), width = 1100, height = 700, res = 130)
par(mar = c(4.5, 4.5, 3.5, 1))
m <- tasa_por("origen")
bp <- barplot(m, beside = TRUE, col = c(COL_MED, COL_GEO), ylim = c(0, 100),
              ylab = "% que acierta (nivel persona)", xlab = "Lugar de origen",
              main = "Tasa de acierto de Carrea por origen (O2)")
text(bp, m, labels = sprintf("%.0f%%", m), pos = 3, cex = 0.8)
legend("topright", c("Cuerda medida", "Cuerda geometrica"),
       fill = c(COL_MED, COL_GEO), bty = "n")
dev.off()
...
bx <- rbind(cbind(long("acierto_med"), modelo = "Medida"),
            cbind(long("acierto_geo"), modelo = "Geometrica"))
bx$cat <- factor(paste(bx$modelo, bx$grupo),
                 levels = c("Medida Acierto", "Medida Fallo",
                            "Geometrica Acierto", "Geometrica Fallo"))
cols4 <- c("#1A9850", "#D73027", "#1A9850", "#D73027")

png(file.path(dir_fig, "demo130_O2_aciertos_estatura_edad.png"), width = 1300, height = 650, res = 130)
par(mfrow = c(1, 2), mar = c(7, 4.5, 3, 1))
boxplot(estatura ~ cat, data = bx, col = cols4, las = 2, xlab = "",
        ylab = "Estatura (m)", main = "Estatura: acierto vs fallo (O2)")
boxplot(edad ~ cat, data = bx, col = cols4, las = 2, xlab = "",
        ylab = "Edad (años)", main = "Edad: acierto vs fallo (O2)")
par(mfrow = c(1, 1))
dev.off()

cat("\nFiguras guardadas en", dir_fig, "(demo130_O2_*.png)\n")
